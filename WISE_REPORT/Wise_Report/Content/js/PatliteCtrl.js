app.controller('Wise_ReportCtrl', function ($http, $scope, $interval, $timeout) {
    ...
    $scope.write = $('#write').val()

    console.log($scope.isadmin + $scope.write)
    ...
    $scope.LoadMachine = function () {
        ...
        stop = $interval(function () {
            $http.post('/api/Api_Machine/GETDATA', thamso).then(function (response) {
                ...
                //    $scope.data[i].SHORTNAME = $scope.data[i].WDT_USERNAME.replace($scope.data[i].GROUP_NAME + "-", "") == "L" ? "LEFT" : "RIGHT"
                //}
            })
        }, 1000)
    }
    ...
    $scope.UpdateNG = function () {
        ...
        $http.post('/api/Api_Setup/UpdateNG',data_update).then(function (response) {
            if (response.data.indexOf("thành công") > 0) {
                $('#InputNG').modal('hide')
            } else {
                ErrorSystem(response.data)
