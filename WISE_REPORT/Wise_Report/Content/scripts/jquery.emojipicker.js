  ...
  $.extend(EmojiPicker.prototype, {
    ...
    iconClicked : function() {
      if ( this.$picker.is(':hidden') ) {
        this.show();
        if (this.$picker.find('.search input').length > 0 || this.$picker.find('.search textarea').length > 0) {
            this.$picker.find('.search input').focus();
            this.$picker.find('.search textarea').focus();
        }
      } else {
        this.hide();
      }
    ...
    emojiClicked: function(e) { var clickTarget = $(e.target);
      ...
      event.initEvent("input","textarea", true, true);
