...
BEGIN
  ...
  IF @SortDirection NOT IN (N'ASCENDING', N'DESCENDING')
  BEGIN
    THROW 51004, 'SortDirection must be ASCENDING or DESCENDING.', 1;
  END;

  DECLARE @Offset bigint = (CONVERT(bigint, @PageNumber) - CONVERT(bigint, 1)) * CONVERT(bigint,@PageSize);
  
  DECLARE @NormalizedSearch nvarchar(max) = NULLIF(LTRIM(RTRIM(@Search)), N'');
  ...
  ORDER BY
    CASE WHEN @SortDirection = N'ASCENDING' THEN
      [U].[Username] END ASC,
    CASE WHEN @SortDirection = N'DESCENDING' THEN
      [U].[Username] END DESC,
    [U].[Id] ASC
  OFFSET @Offset ROWS
